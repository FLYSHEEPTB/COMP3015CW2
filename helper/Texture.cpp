...
void Texture::generate(const std::string fileFront, const std::string fileBack,
	...
	const std::string fileTop,
	const std::string fileBottom) {
	// load image data
	// assume images are the same size
	int width, height, channels;
	unsigned char *imageFront =
		stbi_load(fileFront.c_str(), &width, &height, &channels, 0);
	unsigned char *imageBack =
		stbi_load(fileBack.c_str(), &width, &height, &channels, 0);
	unsigned char *imageLeft =
		stbi_load(fileLeft.c_str(), &width, &height, &channels, 0);
	unsigned char *imageRight =
		stbi_load(fileRight.c_str(), &width, &height, &channels, 0);
	unsigned char *imageTop =
		stbi_load(fileTop.c_str(), &width, &height, &channels, 0);
	unsigned char *imageBottom =
		stbi_load(fileBottom.c_str(), &width, &height, &channels, 0);

	// if successfully loaded cubemap images
	if (imageFront && imageBack && imageLeft && imageRight && imageTop &&
		imageBottom) {
		// generate texture
		glGenTextures(1, &mTextureID);
		...
		glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_Z, 0, GL_RGB, width, height, 0,
			GL_RGB, GL_UNSIGNED_BYTE, imageBack);
		glTexImage2D(GL_TEXTURE_CUBE_MAP_NEGATIVE_Z, 0, GL_RGB, width, height, 0,
			GL_RGB, GL_UNSIGNED_BYTE, imageFront);
		...
		std::cout << "Unable to load cubemap images starting with: " << fileFront
			<< std::endl;
	}
}

void Texture::generate(GLuint textureId, GLuint target)
{
